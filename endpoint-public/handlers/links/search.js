...
const search = function(req, callback, olderLinks = false) {
  ...
  if (!week || !year) {
    week = weeklyCalendarHelper.weekHelper.getWeekNumber(now);
  ...
  query.populate("user", "username").sort({ createdOn: -1 });
  ...
  query.exec(function(err, data) {
    if (err) {
      ...
      const returnData = {
        ...
        week: olderLinks
          ? weeklyCalendarHelper.weekHelper.getWeekNumber(now)
          : week,
...
function getFeed(data, week, year) {
  ...
  const feedId = isNow
    ? `${config.clientDomain}?feed=rss`
    : `${config.clientDomain}week/${week}/year/${year}?feed=rss`;
  const feedTitle = isNow
    ? `dotNET Weekly | A free weekly newsletter on .NET latest`
    : `dotNET Weekly - Week ${week} Year ${year}`;
