...
const updateUser = function(data, updatedUser, callback) {
	...
	if (updatedUser.email !== data.email) {
		...
		sendUpdateEmail(updatedUser, token)
			.then(() => {
				updateUserAction(
					data,
					updatedUser,
					callback,
					`Please visit ${updatedUser.email} to update your email.`
				);
			})
			.catch(error => {
				callback.onSuccess({
...
const sendUpdateEmail = function(user, token) {
	return new Promise((resolve, reject) => {
		axios
			...
			.then(response => {
				...
				emailSender.send(userSubstitutions, '[Call to action] Update your email', response.data.data);
				resolve();
			})
			.catch(error => {
				reject(error);
